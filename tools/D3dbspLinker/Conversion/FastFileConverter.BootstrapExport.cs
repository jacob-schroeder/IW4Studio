...
internal static partial class FastFileConverter
{
    ...
    internal static void ExportSourceAssets(string input, string imageLibrary, string output,
        IReadOnlyList<string> modelNames, IReadOnlyList<string> materialNames, bool bootstrap = false, string? dependencyFastFile = null)
    {
        ...
        {
            if (provider.Asset is not GfxImageAsset image || provider.IsReferencePlaceholder) continue;
            ...
            if (zone is not null) imageResolvers.TryAdd(image, zone.LoadResult.ImagePayloadResolver);
        }
                        ...
                        case GfxImageAsset image:
                            if (imageResolvers.TryGetValue(image, out IGfxImagePayloadResolver? resolver))
                                ...
                                new ImageExchange().UnlinkNative(staging, image,
                                    sourceMaterials.ImageStreamPayloads.GetValueOrDefault(AssetKey.FromDefinition(image)));
                            break;
                        default:
