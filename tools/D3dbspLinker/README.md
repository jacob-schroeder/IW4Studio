...
- `export-assets <linked.ff> <existing-raw-root> <new-output-directory> --xmodel <name> [--material <name>]...` extracts requested complete model/material graphs. It opens the input in isolation; optional `--dependencies <official.ff>` supplies a native dependency workspace during extraction. Shared material/shader/image references can also resolve from the disk library and included bootstrap. Missing complete dependencies fail explicitly. It includes their native geometry, physics, material, technique, shader, and image sources; native images retain their original descriptors and exact stream part bytes. Extraction needs the original adjacent image packages; normal builds only read exported files.
