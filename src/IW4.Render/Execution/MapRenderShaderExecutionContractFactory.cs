...
internal static class MapRenderShaderExecutionContractFactory
{
    internal static MapRenderShaderExecutionContract Create(
        MaterialAsset? material,
        MaterialTechniqueSetAsset? techset,
        IMaterialExecutionLookup lookup,
        MapRenderShaderExecutionPassSelection selectedPass,
        ...
        IReadOnlyList<MapRenderShaderVertexInputBinding>?
            explicitVertexInputs = null)
    {
        ...
        rendererBlockers.AddRange(MapRenderStateExecutionCapability.FindBlockers(selectedPass.State));
                     ...
                     materialSamplers.GroupBy(binding => binding.SamplerDest))
            {
                string[] resourceIdentities = destinationGroup
                    .Select(binding =>
                        ...
                        binding.ResourceBindingIdentity)
    ...
    internal static MapRenderShaderVertexInputBinding[] CreateVertexInputBindings(
        MaterialTechniqueSetAsset? techset,
        ...
        int? fixedVertexSourceBackendRow = null)
    {
        ...
        for (int routeIndex = 0; routeIndex < routeCount; routeIndex++)
        {
            ...
            _ = WorldVertexLayout.TryGetStreamStride(
                effectiveVertexFormat,
                source.StreamIndex,
                out byte stride);
