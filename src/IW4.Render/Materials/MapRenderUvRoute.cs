...
public sealed record MapRenderUvRoute(
    string Label,
    ...
    float AddV)
{
    ...
    public string Formula
    {
        get
        {
            ...
            {
                MapRenderUvBaseMode.Stream0BaseVertexSourceStride =>
                    $"{stream} + baseVertex*{stride} + localIndex*{stride} + {offset}",
                MapRenderUvBaseMode.Stream0GlobalIndexSourceStride =>
    ...
    public static MapRenderUvRoute StaticModel(byte texCoordSource) =>
        new(
            "static model",
            "STATIC_XSURFACE",
            texCoordSource,
