...
/// </summary>
public sealed class ShaderProgramResolution
{
    private readonly byte[] _data;
    private readonly byte[] _rootProgramBytes;

    public ShaderProgramResolution(
        XPointerReference pointer,
        MaterialShaderKind kind,
        MaterialShaderAsset? shader,
        ShaderProgramResolutionKind resolutionKind,
        ShaderProgramProviderIdentity? providerIdentity)
    {
        if (!Enum.IsDefined(kind))
            throw new ArgumentOutOfRangeException(nameof(kind));
        if (!Enum.IsDefined(resolutionKind))
            throw new ArgumentOutOfRangeException(nameof(resolutionKind));
        if (resolutionKind == ShaderProgramResolutionKind.CanonicalActiveProvider &&
    ...
    public ShaderProgramResolutionKind ResolutionKind { get; }

    public ShaderProgramProviderIdentity? ProviderIdentity { get; }
