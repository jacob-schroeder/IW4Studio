...
public static class MapRenderSamplerDecoder
{
    ...
    public static MapRenderSamplerState Decode(byte samplerState, byte descriptorPad0F = 0, byte descriptorPad1B = 0)
    {
        int tableIndex = samplerState & 0x1f;
        int filterClass = tableIndex & 0x07;
        int mipClass = (tableIndex & 0x18) >> 3;
        MapRenderTextureFilter mipFilter = DecodeMipFilter(mipClass);
        ...
        {
            case 2:
                minFilter = MapRenderTextureFilter.Linear;
                magFilter = MapRenderTextureFilter.Linear;
                maxAnisotropy = mipFilter == MapRenderTextureFilter.None
                    ? 1
                    : Math.Clamp(DefaultAnisotropyMin, 1, DefaultAnisotropyMax);
                break;

            case 3:
                minFilter = MapRenderTextureFilter.Anisotropic;
                magFilter = MapRenderTextureFilter.Anisotropic;
            ...
            default:
                minFilter = MapRenderTextureFilter.Point;
                magFilter = MapRenderTextureFilter.Point;
                maxAnisotropy = 1;
                break;
        }

        return new MapRenderSamplerState(
            samplerState,
            Ps3SamplerClampMax,
            descriptorPad0F,
            descriptorPad1B,
            ...
            maxAnisotropy,
            DefaultMipLodBias,
            (samplerState & 0x20) == 0 ? MapRenderTextureAddressMode.Wrap : MapRenderTextureAddressMode.Clamp,
            (samplerState & 0x40) == 0 ? MapRenderTextureAddressMode.Wrap : MapRenderTextureAddressMode.Clamp,
            (samplerState & 0x80) == 0 ? MapRenderTextureAddressMode.Wrap : MapRenderTextureAddressMode.Clamp);
    }
    ...
    private static MapRenderTextureFilter DecodeMipFilter(int mipClass)
    {
        return mipClass switch
        {
            0 => MapRenderTextureFilter.None,
            1 => MapRenderTextureFilter.Point,
            2 => MapRenderTextureFilter.Linear,
            _ => MapRenderTextureFilter.None
        };
    }
}
