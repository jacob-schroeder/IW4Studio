      ...
      IDisposable
{
    ...
    public MenuTextResourceResolver(FastFileEditingSession editingSession)
    {
        ...
        _editingSession.AppliedAssetsChanged +=
            EditingSession_AppliedAssetsChanged;
    }
    ...
    public MenuLocalizedTextResolution ResolveText(string authoredText)
    {
        ...
        for (int attempt = 0; attempt < 2; attempt++)
        {
            ...
            _editingSession.Workspace.LoadedZone.Context.AssetPool,
                authoring!,
                authoredText,
                lookupName,
                revision);
        ...
        MenuFontSelectionContext? context = null)
    {
        ...
        MenuFontEnumResolution mapping = MenuFontEnumMapper.Resolve(
            fontEnum,
            context);
        XAssetPool pool = _editingSession.Workspace.LoadedZone.Context.AssetPool;
    ...
    private void EditingSession_AppliedAssetsChanged(
        object? sender,
        EventArgs args)
    {
        ...
        AppliedAssetDefinitionsCapture capture)
    {
        ...
        foreach (AppliedAssetDefinition captured in capture.Definitions)
        {
            if (captured.Definition is not LocalizeAsset localize ||
                string.IsNullOrWhiteSpace(localize.Name))
            {
                continue;
            }
    ...
    private MenuTextResourceRevision CaptureRevision() => new(
        _editingSession.Workspace.LoadedZone.Context.AssetPool.Revision,
