...
public sealed record XAssetPoolFreeContext
{
    public XAssetPoolFreeContext(
        ...
        XAssetRuntimeAllocationKey allocation)
    {
        if (slotAddress.AssetType != assetType)
        {
            throw new ArgumentException(
                $"Slot type {slotAddress.AssetType} does not match pool-free type {assetType}.",
                nameof(slotAddress));
        }
        if (allocation.SlotAddress != slotAddress)
            throw new ArgumentException("Pool-free allocation does not belong to the supplied stable slot.", nameof(allocation));
