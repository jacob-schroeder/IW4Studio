...
{
    public SysFileHandle Sys_OpenFile(string path)
    {
        ...
        try
        {
            return new SysFileHandle(stream, startOffset: 0);
        }
        catch
        {
            stream.Dispose();
    ...
    public SysFileHandle Sys_OpenFile(Stream handle, int startOffset = 0)
    {
        ...
        return new SysFileHandle(handle, startOffset);
    }

    public int Sys_Read(SysFileHandle file, Span<byte> destination)
    {
        ArgumentNullException.ThrowIfNull(file);
        return file.Handle.Read(destination);
    }

    public byte[] Sys_Read(SysFileHandle file, int byteCount)
    {
        ArgumentNullException.ThrowIfNull(file);
        if (byteCount < 0)
    ...
    public byte[] Sys_ReadToEnd(SysFileHandle file)
    {
        ArgumentNullException.ThrowIfNull(file);
        ...
        if (handle.CanSeek)
        {
            handle.Position = file.File.StartOffset;
            long byteCount = handle.Length - file.File.StartOffset;
    ...
    public void Sys_CloseFile(SysFileHandle file)
    {
        ArgumentNullException.ThrowIfNull(file);
        file.Dispose();
    }
