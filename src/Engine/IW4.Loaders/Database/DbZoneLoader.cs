...
public sealed class DbZoneLoader
{
        ...
        DbRuntime? runtime = null)
    {
        ...
        using SysFileHandle sysFile = _fileSystem.Sys_OpenFile(stream);
    ...
    // The caller owns sysFile for this overload. The byte[] overload above
    // creates and closes its temporary handle after the load completes.
    public LoadedXZone DB_LoadXZone(
        DbFile file,
        SysFileHandle sysFile,
        XZoneInfo zoneInfo,
    ...
    {
        ArgumentNullException.ThrowIfNull(file);
        ArgumentNullException.ThrowIfNull(sysFile);
        ...
        XFileLoadState loadState = DB_InitLoadXFile(file, sysFile, context);
    ...
    public XFileLoadState DB_InitLoadXFile(
        DbFile file,
        SysFileHandle sysFile,
        DbLoadContext context)
    {
        ...
        ArgumentNullException.ThrowIfNull(sysFile);
        ArgumentNullException.ThrowIfNull(context);
        if (!ReferenceEquals(file.SysFile, sysFile.File))
            throw new ArgumentException("The DBFile descriptor does not match the open handle.", nameof(sysFile));

        byte[] buffer = _fileSystem.Sys_ReadToEnd(sysFile);
        ...
        XZone zone)
    {
        ...
        ZoneObjectObservation? observation = context.ZoneObjectCapture is null
            ? null
