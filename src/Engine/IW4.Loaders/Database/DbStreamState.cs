...
internal readonly record struct CStringMaterializationHandle(long Occurrence);
...
public sealed class DbStreamState : IXZoneRuntimeMemory
{
    ...
    public XBlockAddress AllocateInsertPointerCell()
    {
        ...
        CaptureBridge?.RecordDestination(
            ...
            ZoneMaterializationKind.InsertCell);
        ...
        int byteCount)
    {
        ...
        if (CurrentBlock is XFileBlockType.RUNTIME or XFileBlockType.VIRTUAL)
        {
            ...
            Write(zeros);
            CaptureBridge?.RecordDestination(byteCount, destinationAddress, ConsumePendingMaterializationAlignment(CurrentBlock), CurrentBlock == XFileBlockType.RUNTIME ? ZoneMaterializationKind.RuntimeZeroFill : ZoneMaterializationKind.VirtualReservation);
            return zeros;
        }
        ...
        Write(bytes);
        CaptureBridge?.RecordLoad(cursor, sourceOffset, byteCount, destinationAddress, ConsumePendingMaterializationAlignment(CurrentBlock), ZoneMaterializationKind.StreamCopy);
        return bytes;
    }
        ...
        out XBlockAddress address)
    {
        ...
        if (CurrentBlock is XFileBlockType.RUNTIME or XFileBlockType.VIRTUAL)
        {
            byte[] zeros = new byte[byteCount];
            Write(zeros);
            CaptureBridge?.RecordDestination(byteCount, address, ConsumePendingMaterializationAlignment(CurrentBlock), CurrentBlock == XFileBlockType.RUNTIME ? ZoneMaterializationKind.RuntimeZeroFill : ZoneMaterializationKind.VirtualReservation);
            return zeros;
        }
        ...
        Write(bytes.Span);
        CaptureBridge?.RecordLoad(cursor, sourceOffset, byteCount, address, ConsumePendingMaterializationAlignment(CurrentBlock), ZoneMaterializationKind.StreamCopy);
        return bytes;
    }
