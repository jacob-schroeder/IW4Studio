...
/// <summary>
/// Editor-facing validation with stable Menu-node paths. The emitter remains
/// the final wire-contract validator; these diagnostics keep invalid staged
/// values addressable by the Properties UI before export.
/// </summary>
internal static class MenuEditorValidation
{
    ...
    public static IReadOnlyList<AssetValidationIssue> Combine(
        IReadOnlyList<AssetValidationIssue> editorIssues,
        IReadOnlyList<EmissionError> emissionIssues)
    {
        ArgumentNullException.ThrowIfNull(editorIssues);
        ArgumentNullException.ThrowIfNull(emissionIssues);
        var issues = new List<AssetValidationIssue>(editorIssues);
        foreach (EmissionError issue in emissionIssues)
        {
            var converted = new AssetValidationIssue(
                issue.Path,
                issue.Message,
                AssetValidationSeverity.Error);
            if (!issues.Any(existing =>
                    existing.FieldPath == converted.FieldPath &&
                    existing.Message == converted.Message))
            {
                issues.Add(converted);
            }
        }
