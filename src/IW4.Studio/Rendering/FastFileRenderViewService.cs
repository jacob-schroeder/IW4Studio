...
public sealed class FastFileRenderViewService : IDisposable
{
    ...
    public FastFileRenderViewService(IMapRenderSceneBuilder sceneBuilder)
    {
        ArgumentNullException.ThrowIfNull(sceneBuilder);
        _sceneBuilder = sceneBuilder;
    }

    public RenderViewSceneBuildResult BuildScene(
        FastFileWorkspace workspace,
        long snapshotRevision = 0,
        Action<string>? progress = null)
    {
        ThrowIfDisposed();
        return BuildSceneCore(
            workspace,
            snapshotRevision,
            progress,
            interactiveOpenGl: false,
        ...
        CancellationToken buildCancellationToken)
    {
        ...
        LoadedXZone loadedZone = target.LoadResult;
        ...
        if (!ReferenceEquals(
            ...
            !ReferenceEquals(
                loadedZone.Context.AssetRuntimeLifecycle,
                workspace.Runtime.AssetRuntimeLifecycle))
        {
            throw new InvalidOperationException(
                "The target zone's loader context is not bound to this workspace runtime.");
        }
            ...
            buildProgress)
        {
            ...
            BuildProfile = interactiveOpenGl
        ...
        RenderSceneSnapshot sceneSnapshot =
            interactiveOpenGl
                ? RenderSceneSnapshotBuilder.CreateInteractiveOpenGl(
                    scene,
                    snapshotRevision)
                : RenderSceneSnapshotBuilder.Create(
                    scene,
                    snapshotRevision);
        buildCancellationToken.ThrowIfCancellationRequested();
    ...
    public void Dispose()
    {
        ...
        _ = Task.WhenAll(activeBuilds).ContinueWith(
            completed =>
            {
