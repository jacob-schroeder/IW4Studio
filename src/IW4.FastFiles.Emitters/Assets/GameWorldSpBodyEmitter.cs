...
public sealed class GameWorldSpBodyEmitter : IXAssetBodyEmitter
{
    ...
    private static void ValidatePath(PathData path, List<EmissionError> errors, int? rowIndex)
    {
        ...
        if (path.ChainNodeCount > int.MaxValue || path.ChainNodeCount != (uint)path.NodeForChainNode.Count || path.NodeCount != (uint)path.ChainNodeForNode.Count) errors.Add(Error("path.chainNodes", "Path chain arrays do not match their serialized counts.", rowIndex));
