...
internal sealed class BrushLightingScene
{
    ...
    internal bool IsInsideSolid(Vector3 point) => _solidBrushes.Any(brush =>
        brush.Faces.All(face => BrushGeometry.Dot(face.Normal, point - face.A) < -RayOffset));
    ...
    internal Vector3 Ambient(Vector3 point, Vector3 normal)
    {
        ...
        foreach (Vector3 direction in _skyDirections)
        {
            ...
            if (cosine > 0)
            {
                sum += SkyRadiance(origin, direction) * cosine;
                totalWeight += cosine;
            }
        }
    ...
    internal Vector3[] AmbientDirections(Vector3 point)
    {
        CancellationToken.ThrowIfCancellationRequested();
        ...
        foreach (Vector3 direction in _skyDirections)
        {
            Vector3 radiance = SkyRadiance(point, direction);
    ...
    internal Vector3 CaptureRadiance(Vector3 origin, Vector3 direction)
    {
        ...
        foreach (var hit in Trace(origin, direction, shadow: false))
        {
            ...
            Vector3 lighting = Ambient(point, normal) + _sunColor *
    ...
    private Vector3 SkyRadiance(Vector3 point, Vector3 direction)
    {
        float transmission = 1;
        ...
        for (int index = 0; index < Polygons.Count; index++)
        {
            ...
            for (int corner = 1; corner < polygon.Vertices.Length - 1; corner++)
            {
                if (!SurfaceRaycast.RayTriangle(origin, direction, polygon.Vertices[0], polygon.Vertices[corner],
                        polygon.Vertices[corner + 1], out float candidate, out _) || candidate <= 0.0001f) continue;
                ...
                if (!IsSky(index))
                {
                    ...
                    bool passes = (shadow ? state.ShadowAlphaTest : state.AlphaTest) switch
                    {
