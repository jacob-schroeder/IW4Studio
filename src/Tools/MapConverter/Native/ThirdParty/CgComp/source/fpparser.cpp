...
void CFPParser::ParseVectorSrc(const char *token,struct nvfx_src *reg)
{
	...
	if(token[0]=='R' || token[0]=='H') {
		...
		// MojoShader's ARB output names externally supplied D3D constants
		// directly as c[n].  They do not have the Cg-generated #var metadata
		// this parser normally uses, so retain the register as an external RSX
		// constant when no internal #const declaration owns it.
