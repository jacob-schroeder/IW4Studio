...
void CCompilerFP::emit_src(struct nvfx_insn *insn,s32 pos,bool *have_const)
{
	...
	switch(src->reg.type) {
		case NVFXSR_INPUT:
			...
			switch(src->reg.index) {
				case NVFX_FP_OP_INPUT_SRC_COL0:
					m_nInputMask |= 0x5;
					break;
				case NVFX_FP_OP_INPUT_SRC_COL1:
					m_nInputMask |= 0xa;
					break;
				case NVFX_FP_OP_INPUT_SRC_FOGC:
					m_nInputMask |= 0x10;
					break;
				default:
					if(src->reg.index>=NVFX_FP_OP_INPUT_SRC_TC(0) && src->reg.index<=NVFX_FP_OP_INPUT_SRC_TC(7))
						m_nInputMask |= (0x4000 << (src->reg.index - NVFX_FP_OP_INPUT_SRC_TC0));
					break;
			}
...
void CCompilerFP::emit_abs(struct nvfx_insn *insn)
{
...
void CCompilerFP::emit_cmp(struct nvfx_insn *insn)
{
	struct nvfx_insn tmp_insn;
	struct nvfx_src predicate = nvfx_src(temp(insn));
	struct nvfx_src result = nvfx_src(temp(insn));
	struct nvfx_src none = nvfx_src(nvfx_reg(NVFXSR_NONE,0));

	tmp_insn = arith(0,predicate.reg,insn->mask,none,none,none);
	tmp_insn.precision = insn->precision;
	tmp_insn.disable_pc = insn->disable_pc;
	emit_insn(&tmp_insn,NVFX_FP_OP_OPCODE_SFL);

	tmp_insn = arith(0,predicate.reg,insn->mask,insn->src[0],predicate,none);
	tmp_insn.precision = insn->precision;
	tmp_insn.disable_pc = insn->disable_pc;
	emit_insn(&tmp_insn,NVFX_FP_OP_OPCODE_SLT);

	tmp_insn = arith(0,result.reg,insn->mask,neg(predicate),insn->src[2],insn->src[2]);
	tmp_insn.precision = insn->precision;
	tmp_insn.disable_pc = insn->disable_pc;
	emit_insn(&tmp_insn,NVFX_FP_OP_OPCODE_MAD);

	tmp_insn = arith(0,result.reg,insn->mask,predicate,insn->src[1],result);
	tmp_insn.precision = insn->precision;
	tmp_insn.disable_pc = insn->disable_pc;
	emit_insn(&tmp_insn,NVFX_FP_OP_OPCODE_MAD);

	tmp_insn = arith_ctor(insn,insn->dst,result,none,none);
	emit_insn(&tmp_insn,NVFX_FP_OP_OPCODE_MOV);
}

void CCompilerFP::emit_sub(struct nvfx_insn *insn)
{
	struct nvfx_insn tmp_insn;
	struct nvfx_src none = nvfx_src(nvfx_reg(NVFXSR_NONE,0));

	tmp_insn = arith_ctor(insn,insn->dst,insn->src[0],neg(insn->src[1]),none);
	emit_insn(&tmp_insn,NVFX_FP_OP_OPCODE_ADD);
}
